-- ========================================
-- INSTRUCCIONES DE MIGRACIÓN v2.1
-- ========================================

-- ANTES DE CORRER ESTE SCRIPT:
-- 1. HACER BACKUP DE LA BD ACTUAL
-- 2. Ejecutar en phpMyAdmin o línea de comandos

-- OPCIÓN 1: phpMyAdmin
-- - Copiar TODO el contenido de este archivo
-- - Ir a SQL → Pegar → Ejecutar

-- OPCIÓN 2: Línea de comandos
-- mysql -u usuario -p nombre_bd < migrations_v2.1.sql

-- OPCIÓN 3: Dentro de PHP
-- $sql = file_get_contents('database/migrations_v2.1.sql');
-- $pdo->exec($sql);
...
ADD COLUMN `activo` TINYINT(1) DEFAULT 1 COMMENT 'Soft delete: 1=activo, 0=inactivo' 
...
-- ========================================
-- 3. CREAR TABLA 'imagenes_productos'
-- ========================================
...
COMMENT='Imágenes múltiples de productos';
...
BEGIN
    ...
    -- Obtener parámetros configurables de la tabla parametros
    SELECT COALESCE(valor, 0) INTO v_precio_pla_kg 
      FROM parametros WHERE clave = 'precio_pla_kg' LIMIT 1;
    
    SELECT COALESCE(valor, 0) INTO v_costo_luz_kwh 
      FROM parametros WHERE clave = 'costo_luz_kwh' LIMIT 1;
    
    SELECT COALESCE(valor, 0) INTO v_hora_maquina 
      FROM parametros WHERE clave = 'hora_maquina' LIMIT 1;
    
    SELECT COALESCE(valor, 50) INTO v_ganancia_pct 
      FROM parametros WHERE clave = 'ganancia_porcentaje' LIMIT 1;
    
    -- Conversiones a unidades de cálculo
...
-- ========================================
-- 5. CREAR VISTA 'v_productos_con_costo'
-- ========================================
DROP VIEW IF EXISTS `v_productos_con_costo`;

CREATE ALGORITHM=UNDEFINED DEFINER=`u736179347_db_cotiza3d`@`127.0.0.1` 
SQL SECURITY DEFINER VIEW `v_productos_con_costo` AS 
SELECT 
    p.id,
    p.nombre,
    p.descripcion,
...
CREATE ALGORITHM=UNDEFINED DEFINER=`u736179347_db_cotiza3d`@`127.0.0.1` 
SQL SECURITY DEFINER VIEW `v_clientes_activos` AS 
SELECT 
    c.id,
    c.nombre,
    c.email,
...
-- ========================================
-- 7. ACTUALIZAR ÍNDICES para búsqueda rápida (FULLTEXT)
-- ========================================
ALTER TABLE `productos` 
ADD FULLTEXT INDEX `ft_nombre_descripcion` (`nombre`, `descripcion`);

ALTER TABLE `clientes` 
ADD FULLTEXT INDEX `ft_nombre_empresa` (`nombre`, `empresa`);
...
-- Ahora puedes usar:
-- - Campos 'activo' en clientes y productos
-- - Nueva tabla 'imagenes_productos'
-- - Procedimiento 'sp_calcular_costo_producto'
-- - Vistas 'v_productos_con_costo' y 'v_clientes_activos'
-- - Índices FULLTEXT para búsquedas rápidas
